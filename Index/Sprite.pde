class Sprite{
	...
	void animate (String name, float x, float y, float tam) {
		if (this.frames == -1)
			this.frames = jsonSprite.getJSONArray(name).getInt(0);
		if (this.frames >= jsonSprite.getJSONArray(name).getInt(0)
		&& this.frames <= jsonSprite.getJSONArray(name).getInt(1))
